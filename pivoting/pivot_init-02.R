...
l.etlink <- c("cc","fo","so","lm")

# dat1$i.cpl2 %>% c(est1$i.cpl2) %>% unique %>% sort %>% {.[-1]} %>% dput #remove the "" #(with combinations)
l.cpl <- c("a","ai","aqi","d","dn","e","n","o","q","qo","r","s","sq","t","tq","xd","xt")
...
h.clean <- function(x) {if (mode(x)=="character") #helper fun to clean up a vector: replace NAs (and from a char vec also remove line breaks)
  ret <- trimws(ifelse(is.na(x),"",gsub("[\r\n]", " ", x))) else ifelse(is.na(x), 0L, x)            
  }

h.deblank <- function(x) x[x!=""] #helper to remove blank values from a char vector
